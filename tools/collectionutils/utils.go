...
func ForEachB[T any](collection []T, fn func(T) bool) {
	for i := range collection {
		if !fn(collection[i]) {
			return
		}
...
func ForEachBP[T any](collection []T, fn func(*T) bool) {
	for i := range collection {
		if !fn(&collection[i]) {
			return
		}
...
func ForEachE[T any](collection []T, fn func(T) error) error {
	for i := range collection {
		if err := fn(collection[i]); err != nil {
			return err
		}
	}
...
func ForEachEP[T any](collection []T, fn func(*T) error) error {
	for i := range collection {
		if err := fn(&collection[i]); err != nil {
			return err
		}
...
func Find[T any](s []T, equal func(T) bool) (T, bool) {
	for i := range s {
		if equal(s[i]) {
			return s[i], true
		}
...
func FindP[T any](s []T, equal func(*T) bool) (*T, bool) {
	for i := range s {
		if equal(&s[i]) {
			return &s[i], true
		}
