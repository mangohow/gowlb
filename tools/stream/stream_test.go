...
func TestForEach(t *testing.T) {
	t.Run("process all elements", func(t *testing.T) {
		counter := 0
		ForEachB([]int{1, 2, 3}, func(n int) bool {
			counter++
			return true
		})
		if counter != 3 {
	...
	t.Run("stop on first false", func(t *testing.T) {
		processed := []string{}
		ForEachB([]string{"a", "b", "c"}, func(s string) bool {
	...
	t.Run("nil slice safety", func(t *testing.T) {
		...
		ForEachB(nilSlice, func(f float64) bool {
...
func TestForEachP(t *testing.T) {
	t.Run("modify through pointer", func(t *testing.T) {
		data := []int{1, 2, 3}
		ForEachBP(data, func(n *int) bool {
			*n *= 2
			return true
		})
	...
	t.Run("handle nil elements", func(t *testing.T) {
		...
		nilCount := 0
		ForEachBP(slice, func(p **int) bool {
			if *p == nil {
				nilCount++
			}
			return true
	...
	t.Run("struct field access", func(t *testing.T) {
		...
		sum := 0
		ForEachBP(records, func(r *Record) bool {
			sum += r.ID
			return true
		})
		if sum != 6 {
